...
{
    return static_cast<uint32_t*>(leaf_node_cell(node, cell_num)) + LEAF_NODE_KEY_SIZE;
}
...
{
    uint8_t value = *((uint8_t*)(static_cast<uint32_t*>(node) + NODE_TYPE_OFFSET));
...
{
    uint8_t value = type;
    *((uint8_t*)(static_cast<uint32_t*>(node) + NODE_TYPE_OFFSET)) = value;
}
...
{
    uint8_t value = *((uint8_t*)(static_cast<uint8_t*>(node) + IS_ROOT_OFFSET));
